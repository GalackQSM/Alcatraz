  ...
  run(message, args) {
    ...
    if (command && command.type != message.client.types.OWNER && !disabledCommands.includes(command.name)) {
      ...
      const embed = new MessageEmbed() 
        ...
        .addField(`${emojis[2]} ${capitalize(COLOR)}`, `\`${commands[COLOR].length}\` commandes`, true)
        ...
        .addField(`${emojis[8]} ${capitalize(MOD)}`, `\`${commands[MOD].length}\` commandes`, true)
        .addField(`${emojis[9]} ${capitalize(ADMIN)}`, `\`${commands[ADMIN].length}\` commandes`, true)
        .addField(`${emojis[10]} ${capitalize(OWNER)}`, `\`${commands[OWNER].length}\` commandes`, true)
        .addField(`<:alcatraz_liens:776109858197209109> Liens`, '**[Ajouter Alcatraz](https://discordapp.com/oauth2/authorize?client_id=774652242787041310&scope=bot&permissions=2146958847) | '+'[Alcatraz Empire](https://discord.gg/aGP5kvxFEc) | '+'[Github](https://github.com/GalackQSM/Alcatraz) | '+'[SiteWeb](https://alcatraz-bot.com) | '+'[Dons](https://www.patreon.com/AlcatrazBot)**')
        .setFooter(config.footer)
        .setTimestamp()
      ...
      const reactions = {
        ...
        '776109859145252935': new MessageEmbed(json).spliceFields(0, 0 , { 
          name: `${emojis[2]} ${capitalize(COLOR)}`, 
          value: commands[COLOR].join(' ')
        }),
