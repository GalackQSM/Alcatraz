  ...
  if (prefixRegex.test(message.content)) {
    ...
    if (command && !disabledCommands.includes(command.name)) {
      ...
      const embed = new MessageEmbed()
        .setTitle(`Salut, je suis ${config.NomBot}. Besoin d'aide?`)
        ...
        .addField('Tu me veux dans ton serveur ?', oneLine`
          ...
          [ici](https://discordapp.com/oauth2/authorize?client_id=${config.BotID}&scope=bot&permissions=403008599)!
        `)
        .addField('Support', oneLine`
          Si tu as des questions, des suggestions ou tu as trouvé un bogue, n'hésite pas à rejoindre le serveur support: [${config.NomServeur}](${config.Support}) !
        `)
